...
seg_list = jieba.analyse.extract_tags(r"多纳塔斯•斯坦科维休斯   Donatas Stankevicius (立陶宛\5.8 古崖居-妫河公园-世园会\Donatas Stankevicius-立陶宛-2018年5")  # 搜索引擎模式
print("默认：{}".format(' |  '.join(seg_list)))

import time

start = time.time()
words = pseg.cut(r"多纳塔斯•斯坦科维休斯   Donatas Stankevicius (立陶宛\5.8 古崖居-妫河公园-世园会\Donatas Stankevicius-立陶宛-2018年52012年", use_paddle=True)  # 搜索引擎模式
name = []
for word, flag in words:
    if flag == 'nr' and len(word) > 1:
        print('flag:{}  word:{}'.format(flag, word))
end = time.time()
cost = end - start
print("开启paddle耗时%f"%cost)
